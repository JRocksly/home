...
angular.module('static').config(['panelsProvider', function (panelsProvider) {
    ...
    panelsProvider
        .add({
            ...
            position: 'left',
            size: '30%',
            templateUrl: 'directive/panels/chartsListPanel.html',
            controller: 'leftPanelCtrl'
        });
        
}]);

angular.module('static').controller('panelController',['$scope', 'panels', function ($scope, panels) {

    $scope.chartsOpen = function () {
        $scope.$broadcast('openChartsPanel', {});
    };

}]);

angular.module('static').controller('leftPanelCtrl',['$scope', 'panels', function ($scope, panels) {

    $scope.$on('openChartsPanel', function(event, args) {
        panels.open('charts');
    });
